...
- SQLite 固有例外は StoreError(E_STORE)に変換して返す(規約2。素通し禁止)。
- 変更系メソッドは 1 メソッド = 1 トランザクション。失敗時はロールバックし、
  正本と履歴の不整合を残さない(詳細設計書 6.1)。
"""
...
class SQLiteStoreAdapter(StorePort):
    ...
    def save_raw_text(self, raw: RawText) -> None:
        try:
            ...
            raise StoreError(f"原文の保存に失敗しました: {exc}") from exc
    ...
    def save_candidate(self, fact: Fact) -> None:
        try:
            ...
            raise StoreError(f"candidate の保存に失敗しました: {exc}") from exc
    ...
    def find_candidates(self) -> list[Revision]:
        try:
            ...
            raise StoreError(f"candidate の検索に失敗しました: {exc}") from exc
    ...
    def promote_to_active(self, fact_id: str) -> Fact:
        try:
            ...
            raise StoreError(f"正本化に失敗しました: {exc}") from exc
    ...
    def mark_rejected(self, fact_id: str, reason: str | None = None) -> None:
        try:
            ...
            raise StoreError(f"却下の記録に失敗しました: {exc}") from exc
    ...
    def supersede(self, old_fact_id: str, new_fact: Fact) -> None:
        try:
            ...
            raise StoreError(f"置換に失敗しました: {exc}") from exc
    ...
    def find_active(self, tag: str | None = None) -> list[Fact]:
        try:
            ...
            raise StoreError(f"正本の検索に失敗しました: {exc}") from exc
    ...
    def find_active_by_topic(self, topic_tag: str) -> list[Fact]:
        try:
            ...
            raise StoreError(f"トピック検索に失敗しました: {exc}") from exc
    ...
    def get_active(self, fact_id: str) -> Fact | None:
        try:
            ...
            raise StoreError(f"正本の取得に失敗しました: {exc}") from exc
    ...
    def get_revisions(self, fact_id: str) -> list[Revision]:
        try:
            ...
            raise StoreError(f"履歴の取得に失敗しました: {exc}") from exc
        return [self._row_to_revision(row) for row in rows]
    ...
    def append_audit(self, event: AuditEvent) -> None:
        try:
            ...
            raise StoreError(f"監査ログの記録に失敗しました: {exc}") from exc
    ...
    def get_audit_events(self, target_id: str | None = None) -> list[AuditEvent]:
        try:
            ...
            raise StoreError(f"監査ログの取得に失敗しました: {exc}") from exc
        return [
            AuditEvent(
                log_id=row["log_id"],
